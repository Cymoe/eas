...
export default function BandNameScreen() {
  const [bandName, setBandName] = useState('');
  const [isValid, setIsValid] = useState(false);

  const handleBandNameChange = (text: string) => {
    setBandName(text);
  ...
  const handleContinue = async () => {
    if (isValid) {
      try {
        ...
        // Navigate to the band members screen
        router.push('/onboarding/band-members' as any);
      } catch (error) {
        console.error('Error saving band name:', error);
      }
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        <Text style={styles.title}>What's your Band's name?</Text>
